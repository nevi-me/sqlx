...
impl<DB: Database> SharedPool<DB> {
    pub(super) fn new_arc(
        ...
        capacity.checked_add(WAKE_ALL_PERMITS)
    ...
    pub(super) async fn close(&self) {
        ...
        let _permits = self.semaphore.acquire(
            WAKE_ALL_PERMITS + (self.options.max_connections as usize)
        ).await;
    ...
    pub(super) fn release(&self, mut floating: Floating<'_, Live<DB>>) {
        ...
        // release the permit for the connection
    ...
    pub(super) fn try_increment_size<'a>(&'a self, permit: SemaphoreReleaser<'a>) -> Result<DecrementSizeGuard<'a>, SemaphoreReleaser<'a>> {
        match self
            .size
            .fetch_update(Ordering::AcqRel, Ordering::Acquire, |size| Some(size + 1).filter(|size| size <= &self.options.max_connections))
        {
